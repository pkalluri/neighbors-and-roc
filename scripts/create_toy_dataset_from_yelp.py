...
model_opts.HIDDEN_LAYERS = [150, 75, 75]
# self.BATCH_SIZE = 32
...
# DATA
with open(os.path.join(fixed_settings.DATA_ROOT,'yelp-100.json'), 'r') as file:
...
# print('\n'.join([' '.join(t) for t in zip(sentences, Y, S)]), file=open(os.path.join(fixed_settings.GENERATED_DATA_ROOT, 'tmp.txt'), 'w'))
...
Y_sequences = pad_sequences(Y_sequences, maxlen=out_max_sentence_length, padding='pre')
Y_alternative_sequences = pad_sequences(Y_alternative_sequences, maxlen=out_max_sentence_length, padding='pre')
...
X = data[:,:-2]
Y = data[:,-2]
Y_alternatives = data[:,-1]
# print([[index_to_word[i] for i in row if i!=0] for row in X]) # HELPFUL FOR DEBUGGING
#endregion
basic_training_data = (X_sentences[:model_opts.BASE_NUM_TRAINING_SAMPLES],
...
fresh_training_data = (X_sentences[model_opts.BASE_NUM_TRAINING_SAMPLES:model_opts.BASE_NUM_TRAINING_SAMPLES + NUM_SAMPLES_TO_ADD],
                       Y[model_opts.BASE_NUM_TRAINING_SAMPLES:model_opts.BASE_NUM_TRAINING_SAMPLES + NUM_SAMPLES_TO_ADD])
alternative_training_data = (X_sentences[:NUM_SAMPLES_TO_ADD],
                             Y_alternatives[:NUM_SAMPLES_TO_ADD])
testing_data = (X_sentences[len(X_sentences) - model_opts.NUM_TESTING_SAMPLES:],
                Y[len(X_sentences) - model_opts.NUM_TESTING_SAMPLES:] + Y_alternatives[len(X_sentences) - model_opts.NUM_TESTING_SAMPLES:])
...
if model_opts.EMBEDDINGS_FILENAME:
    model = my_models.seq2seq_model(input_length=X.shape[1],
    ...
    model = my_models.seq2seq_model(input_length=X.shape[1],
                                       embedding_matrix_shape=(in_vocab_size, model_opts.EMBEDDING_SIZE),
                                       hidden_layers=model_opts.HIDDEN_LAYERS, dropout=model_opts.DROPOUT, regularize=model_opts.REGULARIZE)
